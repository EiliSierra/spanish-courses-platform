...
import { MATCH_PAIRS_L13 } from '@/lib/lesson-data-L13'
...
export default function MatchingGameL13({ onComplete }: { onComplete?: () => void }) {
  const total = MATCH_PAIRS_L13.length
  const [score, setScore] = useState(0)
  const [matched, setMatched] = useState<Set<string>>(new Set())
  const [wrongZone, setWrongZone] = useState<string | null>(null)
  const [dragging, setDragging] = useState<string | null>(null)
  const [shuffledLeft] = useState(() => shuffle(MATCH_PAIRS_L13))
  const [shuffledRight] = useState(() => shuffle(MATCH_PAIRS_L13))
  const scoreRef = useRef(0)

  const handleDrop = useCallback(
    (leftText: string, targetRight: string) => {
      const pair = MATCH_PAIRS_L13.find((p) => p.left === leftText)
  ...
  return (
    <section id="matching-game">
      ...
      <p className="text-gray-600 mb-2">Drag each Spanish phrase to its number or time. <strong>Score: {score}/{total}</strong></p>
      ...
      <div className="grid md:grid-cols-2 gap-6 mb-4">
        ...
        <div className="space-y-2">
          <div className="text-sm font-semibold text-gray-500 mb-2">Numbers / Times</div>
          {shuffledRight.map((pair) => {
            const isMatched = matched.has(MATCH_PAIRS_L13.find((p) => p.right === pair.right)?.left ?? '')
              ...
              >
                ...
                {isMatched && (
                  <span className="text-sm font-normal ml-2 text-green-600">
                    &#10003; {MATCH_PAIRS_L13.find((p) => p.right === pair.right)?.left}
                  </span>
                )}
              </div>
            )
