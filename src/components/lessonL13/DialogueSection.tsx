...
import { DIALOGUES_L13 } from '@/lib/lesson-data-L13'

export default function DialogueSectionL13() {
  const [activeDialogue, setActiveDialogue] = useState(0)
  const dialogue = DIALOGUES_L13[activeDialogue]
  ...
  return (
    <section id="dialogues">
      ...
      <p className="text-gray-600 mb-5">Listen to real conversations using numbers, time, and calendar vocabulary.</p>

      <div className="flex gap-2 mb-5">
        {DIALOGUES_L13.map((d, i) => (
          ...
          {dialogue.lines.map((line, i) => (
            <div key={i} className={`flex items-start gap-3 ${line.speaker !== dialogue.lines[0].speaker ? 'flex-row-reverse' : ''}`}>
              <div className={`max-w-[80%] rounded-2xl px-5 py-3 ${
                line.speaker === dialogue.lines[0].speaker ? 'bg-gray-100 rounded-tl-sm' : 'bg-amber-600 text-white rounded-tr-sm'
              }`}>
                <div className={`text-xs font-semibold mb-1 ${line.speaker === dialogue.lines[0].speaker ? 'text-gray-500' : 'text-amber-200'}`}>
                  {line.speaker}
                </div>
