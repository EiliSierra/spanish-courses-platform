...
export default function MatchingGame({ pairs, labels, onComplete }: MatchingGameProps) {
  ...
  return (
    <section id="matching-game">
      ...
      <p className="text-gray-500 dark:text-gray-400 mb-2">Drag each item to its match. <strong>Score: {score}/{total}</strong></p>
            ...
            <div
              ...
              className={`px-4 py-3 rounded-lg font-medium text-center transition-all select-none ${
                matched.has(pair.left)
                  ...
                  : 'bg-white dark:bg-gray-800 border-2 border-blue-300 dark:border-blue-700 text-gray-700 dark:text-gray-200 cursor-grab hover:border-blue-400 dark:hover:border-blue-500 hover:shadow-sm active:cursor-grabbing'
              }`}
            >
              {pair.left}
            </div>
              ...
              <div
                ...
                className={`px-4 py-3 rounded-lg font-medium text-center transition-all ${
                  isMatched
                    ...
                    : isWrong
                      ? 'bg-red-50 dark:bg-red-950 border-2 border-red-400 animate-[shake_0.3s]'
                      : 'bg-gray-50 dark:bg-gray-900 border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-purple-400 hover:bg-purple-50 dark:hover:bg-purple-950'
                }`}
              >
                {pair.right}
                {isMatched && (
      ...
      <button onClick={reset} className="px-5 py-2 bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 rounded-lg hover:bg-gray-200 text-sm font-medium transition-colors">
        Reset Game
      </button>
    </section>
  )
