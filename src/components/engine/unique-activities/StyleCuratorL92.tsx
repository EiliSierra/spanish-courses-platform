...
import { STYLE_CURATOR_CHALLENGES } from '@/lib/lessons/L9.2'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 20000

export default function StyleCuratorL92({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = STYLE_CURATOR_CHALLENGES[round]
  const totalRounds = STYLE_CURATOR_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { description: challenge.description, correct: wasCorrect, chosen, answer: challenge.correctConcept }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctConcept
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="style-curator">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Style Curator</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read each fashion description and pick the correct concept!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128087;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} fashion challenges. Identify the correct term for each style description.</p>
    ...
    return (
      <section id="style-curator">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Fashion expert! You curate like a pro!' : pct >= 50 ? 'Good eye! Some fashion terms need more study.' : 'Keep exploring the world of fashion vocabulary!'}</p>
  ...
  return (
    <section id="style-curator">
      ...
      <div className="bg-rose-50 dark:bg-rose-950 border border-rose-200 dark:border-rose-800 rounded-xl p-5 mb-5">
      ...
      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctConcept
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Correct fashion concept!' : `Correct: ${challenge.correctConcept}`}
        </div>
      )}
    </section>
  )
