...
import { LIVE_COMMENTARY_CHALLENGES } from '@/lib/lessons/L8.5'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 15000

export default function LiveCommentaryL85({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ gameAction: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = LIVE_COMMENTARY_CHALLENGES[round]
  const totalRounds = LIVE_COMMENTARY_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { gameAction: challenge.gameAction, correct: wasCorrect, chosen, answer: challenge.correctPhrase }])
  ...
  if (phase === 'start') {
    return (
      <section id="live-commentary">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Live Commentary Challenge</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">A game action unfolds — pick the correct commentary phrase before time runs out!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">🎙️</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} game actions. Choose the right commentary phrase for each one.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">15 seconds per action — fast, like live commentary!</p>
    ...
    return (
      <section id="live-commentary">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'You\'re a natural commentator! Ready for the Superclásico!' : pct >= 50 ? 'Good instincts! Practice will sharpen your commentary speed.' : 'Keep watching matches and learning the phrases!'}</p>
          <div className="bg-emerald-50 dark:bg-emerald-950 rounded-xl p-4 mb-4 text-left space-y-2">
            ...
            {answers.map((a, i) => (
              <div key={i} className={`text-sm p-2 rounded-lg ${a.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
                <p className="font-medium">{a.gameAction}</p>
  ...
  return (
    <section id="live-commentary">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Action {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 rounded-xl p-5 mb-5">
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{challenge.gameAction}</p>
        <p className="text-sm text-emerald-700 mt-1 italic">{challenge.english}</p>
      </div>
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Perfect call! You nailed the commentary!' : `Correct: ${challenge.correctPhrase}`}
        </div>
      )}
    </section>
  )
