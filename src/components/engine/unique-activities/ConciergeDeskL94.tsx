...
import { CONCIERGE_DESK_CHALLENGES } from '@/lib/lessons/L9.4'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 20000

export default function ConciergeDeskL94({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = CONCIERGE_DESK_CHALLENGES[round]
  const totalRounds = CONCIERGE_DESK_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { scenario: challenge.guestRequest, correct: wasCorrect, chosen, answer: challenge.correctResponse }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    setFeedback({ correct })
    if (correct) { spawnConfetti(window.innerWidth / 2, 300, 30) }
  ...
  if (phase === 'start') {
    return (
      <section id="concierge-desk">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Concierge Desk</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">A guest approaches the desk with a request. Choose the most professional response!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128718;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} guest requests. Show your hospitality expertise with each response.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per request.</p>
    ...
    return (
      <section id="concierge-desk">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Five-star concierge service! Guests love you!' : pct >= 50 ? 'Good service! A few responses could be more professional.' : 'Keep practicing — great hospitality takes experience!'}</p>
          <div className="bg-blue-50 dark:bg-blue-950 rounded-xl p-4 mb-4 text-left space-y-2">
            ...
            {answers.map((a, i) => (
              <div key={i} className={`text-sm p-2 rounded-lg ${a.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
                ...
                {!a.correct && <p className="text-xs mt-1">Your answer: {a.chosen} — Best response: <span className="font-semibold">{a.answer}</span></p>}
              </div>
            ))}
          </div>
  ...
  return (
    <section id="concierge-desk">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Request {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-xl p-5 mb-2">
        <p className="text-xs font-semibold text-blue-500 uppercase mb-1">Guest Request:</p>
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{challenge.guestRequest}</p>
        {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 italic">{challenge.english}</p>}
      </div>

      <p className="text-xs text-gray-500 dark:text-gray-400 mb-4 text-center">Choose the most professional response:</p>
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Excellent hospitality!' : `Best response: ${challenge.correctResponse}`}
        </div>
      )}
    </section>
  )
