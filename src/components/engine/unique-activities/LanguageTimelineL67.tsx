...
import { LANGUAGE_TIMELINE_CHALLENGES } from '@/lib/lessons/L6.7'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 20000

export default function LanguageTimelineL67({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ word: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = LANGUAGE_TIMELINE_CHALLENGES[round]
  const totalRounds = LANGUAGE_TIMELINE_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { word: challenge.word, correct: wasCorrect, chosen, answer: challenge.correctOrigin }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctOrigin
    setFeedback({ correct })
    ...
    <section id="language-timeline">
      ...
      <div className="grid grid-cols-2 gap-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctOrigin
