  ...
  if (phase === 'start') {
    return (
      <section id="outfit-builder">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Pick the right clothes for each scenario!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128087;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} scenarios. Pick exactly 3 items that fit the situation.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per outfit.</p>
    ...
    return (
      <section id="outfit-builder">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Fashion expert!' : pct >= 50 ? 'Nice picks! A few outfits were off.' : 'Keep practicing your outfits!'}</p>
  ...
  return (
    <section id="outfit-builder">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Outfit {round + 1} of {totalRounds}</span>
      ...
      <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3">Pick 3 items:</p>
          ...
          return (
            <button key={item} onClick={() => toggleItem(item)} disabled={!!feedback}
              className={`px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                show ? isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200'
                  : isSel && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300'
                  : 'border-gray-200 dark:border-gray-700 opacity-40'
                : isSel ? 'border-pink-400 bg-pink-50 dark:bg-pink-950 text-pink-800 dark:text-pink-200 ring-2 ring-pink-200'
                : 'border-gray-200 dark:border-gray-700 hover:border-pink-300 hover:bg-pink-950/50 cursor-pointer'
              }`}>{item}</button>
          )
        })}
      </div>
      ...
      {!feedback && (
        <button onClick={handleSubmit} disabled={selected.size !== 3}
          className={`w-full py-3 rounded-xl font-bold text-sm transition-all ${
            selected.size === 3 ? 'bg-pink-500 text-white hover:bg-pink-600 shadow-lg' : 'bg-gray-200 text-gray-500 dark:text-gray-400 cursor-not-allowed'
          }`}>
