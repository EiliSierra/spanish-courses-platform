...
import { BIAS_DETECTOR_CHALLENGES } from '@/lib/lessons/L7.3'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function BiasDetectorL73({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ headline: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = BIAS_DETECTOR_CHALLENGES[round]
  const totalRounds = BIAS_DETECTOR_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { headline: challenge.headline, correct: wasCorrect, chosen, answer: challenge.correctBias }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctBias
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="bias-detector">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Bias Detector</h2>
        <p className="text-gray-400 mb-4">Read each headline and identify the bias technique or logical fallacy being used!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128270;</div>
          <p className="text-gray-400 mb-2">{totalRounds} headlines. Spot the bias technique in each one.</p>
          <p className="text-sm text-gray-400 mb-6">22 seconds per headline.</p>
          <button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-bold text-lg hover:from-orange-600 hover:to-red-600 transition-all shadow-lg hover:shadow-xl">
            Start Detecting
          </button>
        </div>
      </section>
    ...
    return (
      <section id="bias-detector">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Sharp media critic! You spotted the bias like a pro!' : pct >= 50 ? 'Good instincts! Some bias techniques need more practice.' : 'Keep training your critical eye — bias hides in plain sight!'}</p>
          <div className="bg-orange-950 rounded-xl p-4 mb-4 text-left space-y-2">
            <p className="text-xs font-semibold text-orange-600 uppercase mb-2">Your Results:</p>
            {answers.map((a, i) => (
  ...
  return (
    <section id="bias-detector">
      ...
      <div className="bg-orange-950 border border-orange-800 rounded-xl p-5 mb-5">
        <p className="font-semibold text-gray-200 text-lg italic">{challenge.headline}</p>
        {feedback && <p className="text-sm text-gray-400 mt-2">{challenge.english}</p>}
      </div>

      <p className="text-sm text-gray-400 mb-3 font-medium">What bias technique is being used?</p>
      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctBias
            ...
            <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
              className={`w-full text-left px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                show ? isCorrect ? 'border-green-400 bg-green-950 text-green-200' : isSelected && !isCorrect ? 'border-red-700 bg-red-950 text-red-700' : 'border-gray-700 opacity-40'
                  : 'border-gray-700 hover:border-orange-400 hover:bg-orange-950/50 cursor-pointer'
              }`}>{opt}</button>
          )
        })}
      </div>
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
          {feedback.correct ? 'Correct! You spotted the bias!' : `The bias technique is: ${challenge.correctBias}`}
        </div>
      )}
    </section>
  )
