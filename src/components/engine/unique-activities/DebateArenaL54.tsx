...
import { DEBATE_ARENA_CHALLENGES } from '@/lib/lessons/L5.4'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function DebateArenaL54({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ topic: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = DEBATE_ARENA_CHALLENGES[round]
  const totalRounds = DEBATE_ARENA_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { topic: challenge.topic, correct: wasCorrect, chosen, answer: challenge.correctMarker }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctMarker
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="debate-arena">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Debate Arena</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">You&apos;re in a heated debate! Pick the correct discourse marker to complete each argument.</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#9876;&#65039;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} debate arguments. Choose the right discourse marker for each.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">22 seconds per argument.</p>
          <button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-rose-500 to-orange-500 text-white rounded-xl font-bold text-lg hover:from-rose-600 hover:to-orange-600 transition-all shadow-lg hover:shadow-xl">
            Enter the Arena
          </button>
        </div>
      </section>
    ...
    return (
      <section id="debate-arena">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Master debater! Your argumentation is razor-sharp.' : pct >= 50 ? 'Good instincts! Some markers need more practice.' : 'Keep practicing — discourse markers take time to master!'}</p>
          <div className="bg-rose-50 dark:bg-rose-950 rounded-xl p-4 mb-4 text-left space-y-2">
            <p className="text-xs font-semibold text-rose-600 uppercase mb-2">Your Arguments:</p>
            {answers.map((a, i) => (
              <div key={i} className={`text-sm p-2 rounded-lg ${a.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
                ...
                {!a.correct && <p className="text-xs mt-1">Your choice: {a.chosen} — Correct: <span className="font-semibold">{a.answer}</span></p>}
              </div>
            ))}
          </div>
  ...
  return (
    <section id="debate-arena">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Argument {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-gradient-to-r from-rose-50 to-orange-50 border border-rose-800 rounded-xl p-5 mb-2">
        <div className="flex items-center gap-2 mb-2">
          <span className="text-xs font-semibold text-rose-600 uppercase">Debate Topic:</span>
          <span className="text-sm font-bold text-gray-700 dark:text-gray-200">{challenge.topic}</span>
        </div>
        <p className="text-xs text-orange-600 mb-3">Position: {challenge.position}</p>
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{challenge.sentence}</p>
        {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 italic">{challenge.english}</p>}
      </div>
      <p className="text-xs font-semibold text-rose-500 uppercase mb-3 mt-3">Choose the correct discourse marker:</p>

      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctMarker
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Perfect marker choice!' : `Correct marker: ${challenge.correctMarker}`}
        </div>
      )}
    </section>
  )
