...
export default function PictureMatchL16({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <div id="picture-match" className="py-6">
      ...
      {/* Clue */}
      <div className="bg-rose-50 dark:bg-rose-950 border border-rose-200 dark:border-rose-800 rounded-xl p-5 mb-6">
        <div className="flex items-start gap-3">
          <span className="text-2xl">🗣️</span>
          <div className="flex-1">
            <p className="text-sm text-rose-600 font-semibold mb-1">Listen to the description:</p>
            <p className="text-lg font-medium text-gray-700 dark:text-gray-200 italic">&ldquo;{currentRound.clue}&rdquo;</p>
          </div>
          <button
            onClick={replayAudio}
      ...
      {/* Family cards */}
      <p className="text-sm text-gray-500 dark:text-gray-400 font-medium mb-3">Which family matches this description?</p>
      <div className="grid grid-cols-2 gap-4">
        {PICTURE_MATCH_FAMILIES.map((family) => {
          ...
          if (isCorrectAnswer && feedback) {
            cardClass += 'border-green-500 bg-green-50 dark:bg-green-950 ring-2 ring-green-300 scale-[1.02]'
          } else if (isWrongSelection) {
            cardClass += 'border-red-400 bg-red-50 dark:bg-red-950 ring-2 ring-red-300'
          } else if (feedback) {
            cardClass += 'border-gray-200 dark:border-gray-700 bg-gray-50 opacity-50'
          } else {
            cardClass += 'border-gray-200 dark:border-gray-700 bg-white hover:border-rose-300 hover:shadow-md'
          }
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400">{family.description}</p>
