...
import { HOTEL_CHECKIN_SCENARIOS } from '@/lib/lessons/L2.4'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 15000

export default function HotelCheckinL24({ onComplete }: { onComplete?: () => void }) {
  ...
  const scenario = HOTEL_CHECKIN_SCENARIOS[round]
  const totalRounds = HOTEL_CHECKIN_SCENARIOS.length
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === scenario.correctRoom
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="hotel-checkin">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Hotel Check-in Simulator</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read what the guest needs and pick the right room!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127976;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} guests. Read their request and assign the correct room.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">15 seconds per guest.</p>
    ...
    return (
      <section id="hotel-checkin">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Star receptionist!' : pct >= 50 ? 'Good job! A few rooms were wrong.' : 'Keep practicing!'}</p>
  ...
  return (
    <section id="hotel-checkin">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Guest {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <div className="bg-cyan-50 dark:bg-cyan-950 border border-cyan-200 dark:border-cyan-800 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            <span className="text-2xl">&#128100;</span>
            <div>
              <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{scenario.guestRequest}</p>
              {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 italic">{scenario.english}</p>}
            </div>
          </div>
        </div>
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3">Which room do they need?</p>
        <div className="grid grid-cols-2 gap-3">
          {scenario.options.map((opt) => {
            const isCorrect = opt === scenario.correctRoom
              ...
              <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
                className={`px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                  show ? isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300' : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : 'border-gray-200 dark:border-gray-700 hover:border-cyan-400 hover:bg-cyan-50 dark:hover:bg-cyan-50 dark:hover:bg-cyan-950/50 cursor-pointer'
                }`}>{opt}</button>
            )
          })}
        </div>
        {feedback && (
          <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
            {feedback.correct ? 'Perfect!' : `Correct: ${scenario.correctRoom}`}
          </div>
        )}
      </div>
    </section>
