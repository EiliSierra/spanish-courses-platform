...
import { LECTURE_SUMMARIZER_CHALLENGES } from '@/lib/lessons/L5.7'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 25000

export default function LectureSummarizerL57({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ passage: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = LECTURE_SUMMARIZER_CHALLENGES[round]
  const totalRounds = LECTURE_SUMMARIZER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { passage: challenge.passage, correct: wasCorrect, chosen, answer: challenge.correctSummary }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctSummary
    setFeedback({ correct })
    ...
    <section id="lecture-summarizer">
      ...
      <div className="space-y-3">
        {challenge.options.map((opt, idx) => {
          const isCorrect = opt === challenge.correctSummary
