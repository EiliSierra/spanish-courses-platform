  ...
  if (phase === 'start') {
    return (
      <section id="boarding-pass">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read the boarding pass in Spanish and answer the question!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127915;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} boarding passes. Read the info and answer correctly.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per pass.</p>
    ...
    return (
      <section id="boarding-pass">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Expert traveler!' : pct >= 50 ? 'Good reading! A few details missed.' : 'Keep practicing your boarding passes!'}</p>
  ...
  return (
    <section id="boarding-pass">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Pass {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-white rounded-xl border-2 border-dashed border-sky-300 p-5 mb-5 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start mb-4 pt-2">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Pasajero / Passenger</p>
            <p className="text-lg font-bold text-gray-700 dark:text-gray-200">{scenario.passengerName}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Vuelo / Flight</p>
            <p className="text-lg font-bold text-sky-600">{scenario.flight}</p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Origen / From</p>
            <p className="text-sm font-semibold text-gray-700">{scenario.from}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Destino / To</p>
            <p className="text-sm font-semibold text-gray-700">{scenario.to}</p>
          </div>
        </div>
        <div className="grid grid-cols-4 gap-3 bg-sky-50 dark:bg-sky-950/60 rounded-lg p-3">
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Puerta / Gate</p>
            <p className="text-base font-bold text-gray-700 dark:text-gray-200">{scenario.gate}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Asiento / Seat</p>
            <p className="text-base font-bold text-gray-700 dark:text-gray-200">{scenario.seat}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Embarque / Boarding</p>
            <p className="text-base font-bold text-gray-700 dark:text-gray-200">{scenario.boarding}</p>
          </div>
          <div>
            <p className="text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 font-semibold">Salida / Departure</p>
            <p className="text-base font-bold text-gray-700 dark:text-gray-200">{scenario.departure}</p>
          </div>
        </div>
      </div>

      {/* Question */}
      <div className="bg-sky-50 dark:bg-sky-950 border border-sky-200 dark:border-sky-800 rounded-xl p-4 mb-5">
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{scenario.question}</p>
        {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 italic">{scenario.questionEnglish}</p>}
      </div>
