...
export default function HobbyMatchL35({ onComplete }: { onComplete?: () => void }) {
  ...
  if (phase === 'start') {
    return (
      <section id="hobby-match">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read a person&apos;s description and match them to their hobby!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127939;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} people. First, meet them and learn their hobbies. Then identify them!</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per person.</p>
          <button onClick={startStudy} className="px-8 py-3 bg-gradient-to-r from-emerald-500 to-green-500 text-white rounded-xl font-bold text-lg hover:from-emerald-600 hover:to-green-600 transition-all shadow-lg hover:shadow-xl">
            Meet the People
          </button>
        </div>
      </section>
  ...
  if (phase === 'study') {
    return (
      <section id="hobby-match">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Hobby Match</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Study each person and their hobby. You&apos;ll need to match them from Spanish descriptions!</p>
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          {HOBBY_MATCH_CHALLENGES.map((ch) => (
            <div key={ch.correctHobby} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">&#128100;</span>
                <h3 className="font-bold text-emerald-700 dark:text-emerald-400">{ch.correctHobby}</h3>
              </div>
              <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed">{ch.personDescription}</p>
    ...
    return (
      <section id="hobby-match">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Perfect hobby detective!' : pct >= 50 ? 'Good job! A few tricky ones.' : 'Keep practicing!'}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <div className="bg-emerald-50 dark:bg-emerald-950 border border-emerald-200 dark:border-emerald-800 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            <span className="text-2xl">&#128100;</span>
            <div>
              <p className="font-semibold text-gray-800 dark:text-gray-200 text-lg">{challenge.personDescription}</p>
              {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 italic">{challenge.english}</p>}
            </div>
          </div>
        </div>
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3">What is their hobby?</p>
              ...
              <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
                className={`px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                  show ? isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300' : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : 'border-gray-200 dark:border-gray-700 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-50 dark:hover:bg-emerald-950/50 cursor-pointer text-gray-700 dark:text-gray-200'
                }`}>{opt}</button>
            )
          })}
        </div>
