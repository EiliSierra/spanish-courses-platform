...
import { PUN_MASTER_CHALLENGES } from '@/lib/lessons/L7.2'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 20000

export default function PunMasterL72({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ setup: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = PUN_MASTER_CHALLENGES[round]
  const totalRounds = PUN_MASTER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { setup: challenge.setup, correct: wasCorrect, chosen, answer: challenge.correctPunchline }])
    if (round + 1 >= totalRounds) {
      setPhase('results')
      if (scoreRef.current >= 5) {
        spawnConfetti(window.innerWidth / 2, 200, 50)
        setTimeout(() => spawnConfetti(window.innerWidth / 3, 150, 30), 300)
        setTimeout(() => spawnConfetti(window.innerWidth * 2 / 3, 150, 30), 600)
        setTimeout(() => spawnConfetti(window.innerWidth / 2, 100, 40), 900)
        setTimeout(() => spawnConfetti(window.innerWidth / 2, 250, 35), 1200)
        onComplete?.()
      }
    } else {
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctPunchline
    setFeedback({ correct })
            ...
            {answers.map((a, i) => (
              <div key={i} className={`text-sm p-2 rounded-lg ${a.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
                ...
                {a.correct ? (
                  ...
                  <p className="text-xs mt-1">Your answer: {a.chosen} — Correct: <span className="font-semibold">{a.answer}</span></p>
                )}
              </div>
            ))}
          </div>
          <button onClick={startGame} className="px-6 py-2 bg-yellow-600 text-white rounded-lg font-medium hover:bg-yellow-700">Try Again</button>
        </div>
      </section>
    )
  }
