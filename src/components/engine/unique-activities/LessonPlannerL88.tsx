...
import { LESSON_PLANNER_CHALLENGES } from '@/lib/lessons/L8.8'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 25000

export default function LessonPlannerL88({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = LESSON_PLANNER_CHALLENGES[round]
  const totalRounds = LESSON_PLANNER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { scenario: challenge.teachingScenario, correct: wasCorrect, chosen, answer: challenge.correctApproach }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctApproach
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="lesson-planner">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Lesson Planner</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read each teaching scenario and choose the best pedagogical approach. Think like an expert educator!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128218;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} teaching scenarios. Choose the most effective pedagogical approach for each one.</p>
    ...
    return (
      <section id="lesson-planner">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Outstanding pedagogical instincts! You think like a master teacher.' : pct >= 50 ? 'Good teaching sense! Review the approaches you missed.' : 'Keep studying pedagogy — great teaching is a lifelong learning journey!'}</p>
          <div className="bg-amber-50 dark:bg-amber-950 rounded-xl p-4 mb-4 text-left space-y-2">
            <p className="text-xs font-semibold text-amber-600 uppercase mb-2">Your Approaches:</p>
            {answers.map((a, i) => (
  ...
  return (
    <section id="lesson-planner">
      ...
      <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-700 rounded-xl p-5 mb-3">
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{challenge.teachingScenario}</p>
      ...
      <div className="bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-xl p-4 mb-5">
        <p className="text-sm font-medium text-orange-800 dark:text-orange-800 dark:text-orange-200">Which pedagogical approach is most appropriate?</p>
      </div>

      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctApproach
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Excellent pedagogical reasoning!' : `Correct approach: ${challenge.correctApproach}`}
        </div>
      )}
    </section>
  )
