  ...
  if (phase === 'start') {
    return (
      <section id="weather-forecast">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read the weather report and identify the weather AND the season!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#9925;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} cities across Latin America and Spain.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">Select the weather condition and the season. 15 seconds per round.</p>
    ...
    return (
      <section id="weather-forecast">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Expert meteorologist!' : pct >= 50 ? 'Good reading! Try again for a better score.' : 'Keep practicing your weather vocabulary!'}</p>
  ...
  return (
    <section id="weather-forecast">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        {/* City & Forecast */}
        <div className="bg-sky-50 dark:bg-sky-950 border border-sky-200 dark:border-sky-800 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-2 mb-2">
            <span className="text-lg">&#127758;</span>
            <span className="font-bold text-sky-800 dark:text-sky-800 dark:text-sky-200">{scenario.city}, {scenario.country}</span>
          </div>
          <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">&ldquo;{scenario.description}&rdquo;</p>
          {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 italic">{scenario.english}</p>}
        </div>

        {/* Weather Selection */}
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2">What is the weather?</p>
            ...
            return (
              <button key={opt} onClick={() => !feedback && setSelectedWeather(opt)} disabled={!!feedback}
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  showResult
                    ? isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200'
                      : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300'
                      : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : isSelected ? 'border-sky-500 bg-sky-50 dark:bg-sky-950 text-sky-800 dark:text-sky-200 ring-2 ring-sky-300'
        ...
        {/* Season Selection */}
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2">What season?</p>
        <div className="grid grid-cols-2 gap-2 mb-4">
          {scenario.seasonOptions.map((opt) => {
            ...
            return (
              <button key={opt} onClick={() => !feedback && setSelectedSeason(opt)} disabled={!!feedback}
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  showResult
                    ? isCorrect ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-200'
                      : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300'
                      : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : isSelected ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-200 ring-2 ring-emerald-300'
