  ...
  if (phase === 'start') {
    return (
      <section id="weather-forecast">
        ...
        <p className="text-gray-600 mb-4">Read the weather report and identify the weather AND the season!</p>
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <div className="text-5xl mb-4">&#9925;</div>
          <p className="text-gray-600 mb-2">{totalRounds} cities across Latin America and Spain.</p>
    ...
    return (
      <section id="weather-forecast">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">{pct >= 80 ? 'Expert meteorologist!' : pct >= 50 ? 'Good reading! Try again for a better score.' : 'Keep practicing your weather vocabulary!'}</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <div className="bg-sky-50 border border-sky-200 rounded-xl p-4 mb-6">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-bold text-sky-800">{scenario.city}, {scenario.country}</span>
          </div>
          <p className="font-semibold text-gray-800 text-lg">&ldquo;{scenario.description}&rdquo;</p>
          {feedback && <p className="text-sm text-gray-500 mt-1 italic">{scenario.english}</p>}
        </div>

        {/* Weather Selection */}
        <p className="text-sm font-semibold text-gray-500 mb-2">What is the weather?</p>
              ...
              <button key={opt} onClick={() => !feedback && setSelectedWeather(opt)} disabled={!!feedback}
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  showResult
                    ...
                    : isSelected ? 'border-sky-500 bg-sky-50 text-sky-800 ring-2 ring-sky-300'
        ...
        {/* Season Selection */}
        <p className="text-sm font-semibold text-gray-500 mb-2">What season?</p>
        <div className="grid grid-cols-2 gap-2 mb-4">
                ...
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  showResult
                    ? isCorrect ? 'border-emerald-400 bg-emerald-50 text-emerald-800'
                      : isSelected && !isCorrect ? 'border-red-300 bg-red-50 text-red-700'
                      : 'border-gray-200 opacity-40'
                    : isSelected ? 'border-emerald-500 bg-emerald-50 text-emerald-800 ring-2 ring-emerald-300'
