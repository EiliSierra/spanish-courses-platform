  ...
  if (phase === 'start') {
    return (
      <section id="story-builder">
        ...
        <p className="text-gray-600 mb-4">Build a story about your day using the correct preterite form!</p>
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <div className="text-5xl mb-4">&#128214;</div>
          <p className="text-gray-600 mb-2">{totalRounds} steps. Pick the correct past tense sentence for each part of the story.</p>
    ...
    return (
      <section id="story-builder">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">{pct >= 80 ? 'Great storyteller!' : pct >= 50 ? 'Good story! Some verb forms need practice.' : 'Keep working on those verb forms!'}</p>
  ...
  return (
    <section id="story-builder">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500">Step {round + 1} of {totalRounds}</span>
      ...
      {story.length > 0 && (
        <div className="bg-amber-50/50 rounded-lg p-3 mb-4 border border-amber-100">
          ...
          {story.map((s, i) => <span key={i} className="text-sm text-gray-600">{s}{i < story.length - 1 ? '. ' : '...'}</span>)}
        </div>
      )}

      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5">
        <p className="font-semibold text-gray-800 text-lg">{step.prompt}</p>
        {feedback && <p className="text-sm text-gray-500 mt-1 italic">{step.english}</p>}
      </div>
