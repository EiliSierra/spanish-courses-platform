...
import { TECH_SPEC_CHALLENGES } from '@/lib/lessons/L8.3'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function TechSpecL83({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = TECH_SPEC_CHALLENGES[round]
  const totalRounds = TECH_SPEC_CHALLENGES.length
  ...
  if (phase === 'start') {
    return (
      <section id="tech-spec">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Tech Spec Challenge</h2>
        <p className="text-gray-400 mb-4">Read the tech term and pick the correct definition from four options!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128187;</div>
          <p className="text-gray-400 mb-2">{totalRounds} technical terms. Pick the correct definition for each one.</p>
          <p className="text-sm text-gray-400 mb-6">22 seconds per term.</p>
    ...
    return (
      <section id="tech-spec">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Tech expert! You nailed every definition!' : pct >= 50 ? 'Good work! Review a few terms and try again.' : 'Keep studying the tech vocabulary and try again!'}</p>
  ...
  return (
    <section id="tech-spec">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-400">Term {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-blue-950 border border-blue-800 rounded-xl p-5 mb-2">
        <p className="font-semibold text-gray-200 text-lg">{challenge.term}</p>
        <p className="text-sm text-gray-400 mt-1 italic">{challenge.english}</p>
      </div>
      <p className="text-xs text-gray-400 mb-4">Pick the correct definition:</p>
