...
export default function SpeechCrafterL68({ onComplete }: { onComplete?: () => void }) {
  ...
  if (phase === 'start') {
    return (
      <section id="speech-crafter">
        ...
        <p className="text-gray-400 mb-4">Read the speech context and select the most powerful rhetorical phrase. Think like an orator!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127908;</div>
          <p className="text-gray-400 mb-2">{totalRounds} speech scenarios. Choose the best rhetorical phrase for each moment.</p>
    ...
    return (
      <section id="speech-crafter">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'A natural orator! Your rhetoric is powerful.' : pct >= 50 ? 'Good instincts! Study the techniques you missed.' : 'Keep practicing — great speakers are made, not born!'}</p>
          <div className="bg-red-950 rounded-xl p-4 mb-4 text-left space-y-2">
  ...
  return (
    <section id="speech-crafter">
      ...
      <div className="bg-red-950 border border-red-200 rounded-xl p-5 mb-3">
        <p className="font-semibold text-gray-200 text-lg">{challenge.context}</p>
        {feedback && <p className="text-sm text-gray-400 mt-2 italic">{challenge.english}</p>}
      </div>

      <div className="bg-rose-950 border border-rose-800 rounded-xl p-4 mb-5">
            ...
            <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
              className={`w-full text-left px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                show ? isCorrect ? 'border-green-400 bg-green-950 text-green-200' : isSelected && !isCorrect ? 'border-red-700 bg-red-950 text-red-700' : 'border-gray-700 opacity-40'
                  : 'border-gray-700 hover:border-red-400 hover:bg-red-950/50 cursor-pointer'
              }`}>{opt}</button>
          )
        })}
      </div>
