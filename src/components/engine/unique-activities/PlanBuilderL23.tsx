...
import { PLAN_BUILDER_SCENARIOS } from '@/lib/lessons/L2.3'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 15000

export default function PlanBuilderL23({ onComplete }: { onComplete?: () => void }) {
  ...
  const scenario = PLAN_BUILDER_SCENARIOS[round]
  const totalRounds = PLAN_BUILDER_SCENARIOS.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean) => {
    ...
    if (round + 1 >= totalRounds) {
      ...
      setTimeout(() => { setFeedback(null); setSelectedDay(null); setSelectedActivity(null); setRound(r => r + 1) }, 2000)
    }
  ...
  const handleSubmit = useCallback(() => {
    if (feedback || !selectedDay || !selectedActivity) return
    clearTimer()
    const correct = selectedDay === scenario.correctDay && selectedActivity === scenario.correctActivity
    setFeedback({ correct })
    setTimeout(() => advanceRound(correct), 2000)
  }, [feedback, selectedDay, selectedActivity, clearTimer, scenario, advanceRound])

  const startGame = useCallback(() => {
    setPhase('game'); setRound(0); setScore(0); scoreRef.current = 0
    setFeedback(null); setSelectedDay(null); setSelectedActivity(null)
  }, [])
  ...
  if (phase === 'start') {
    return (
      <section id="plan-builder">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read the invitation and identify WHEN and WHAT the plan is!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128197;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} invitations. Read carefully and pick the day and activity.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">15 seconds per round.</p>
    ...
    return (
      <section id="plan-builder">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Social butterfly!' : pct >= 50 ? 'Good planning! Try again.' : 'Keep practicing!'}</p>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <div className="bg-violet-50 dark:bg-violet-950 border border-violet-200 dark:border-violet-800 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{scenario.situation}</p>
              {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 italic">{scenario.english}</p>}
            </div>
          </div>
        </div>
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2">When?</p>
        <div className="grid grid-cols-2 gap-2 mb-4">
          {scenario.dayOptions.map((opt) => {
            const isCorrect = opt === scenario.correctDay
            const isSelected = selectedDay === opt
              ...
              <button key={opt} onClick={() => !feedback && setSelectedDay(opt)} disabled={!!feedback}
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  show ? isCorrect ? 'border-green-400 bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300' : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : isSelected ? 'border-violet-500 bg-violet-950 text-violet-200 ring-2 ring-violet-300' : 'border-gray-200 dark:border-gray-700 hover:border-violet-400 hover:bg-violet-50 dark:hover:bg-violet-950/50 cursor-pointer'
                }`}>{opt}</button>
            )
          })}
        </div>
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-2">What activity?</p>
        <div className="grid grid-cols-2 gap-2 mb-4">
          {scenario.activityOptions.map((opt) => {
            const isCorrect = opt === scenario.correctActivity
            const isSelected = selectedActivity === opt
            const show = feedback !== null
            return (
              <button key={opt} onClick={() => !feedback && setSelectedActivity(opt)} disabled={!!feedback}
                className={`px-3 py-3 rounded-xl border-2 font-medium text-sm transition-all ${
                  show ? isCorrect ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-200' : isSelected && !isCorrect ? 'border-red-400 bg-red-50 dark:bg-red-950 text-red-700 dark:text-red-300' : 'border-gray-200 dark:border-gray-700 opacity-40'
                    : isSelected ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-200 ring-2 ring-emerald-300' : 'border-gray-200 dark:border-gray-700 hover:border-emerald-400 hover:bg-emerald-50 dark:hover:bg-emerald-950/50 cursor-pointer'
                }`}>{opt}</button>
            )
          })}
        </div>
        {!feedback && (
          <button onClick={handleSubmit} disabled={!selectedDay || !selectedActivity}
            className={`w-full py-3 rounded-xl font-bold text-lg transition-all ${
              selectedDay && selectedActivity ? 'bg-gradient-to-r from-violet-500 to-purple-500 text-white hover:from-violet-600 hover:to-purple-600 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 cursor-not-allowed'
            }`}>Confirm Plan</button>
        )}
        {feedback && (
