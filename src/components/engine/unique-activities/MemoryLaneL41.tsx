  ...
  if (phase === 'start') {
    return (
      <section id="memory-lane">
        ...
        <p className="text-gray-600 mb-4">Read each sentence and decide: is it Imperfect or Preterite?</p>
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <div className="text-5xl mb-4">&#128173;</div>
          <p className="text-gray-600 mb-2">{totalRounds} memory scenarios. Pick the correct tense for each one.</p>
    ...
    return (
      <section id="memory-lane">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">{pct >= 80 ? 'Excellent! You really understand the difference!' : pct >= 50 ? 'Good job! Keep practicing the imperfect vs. preterite contrast.' : 'Keep working on distinguishing imperfect from preterite!'}</p>
          <div className="bg-purple-50 rounded-xl p-4 mb-4 text-left max-h-64 overflow-y-auto">
            <p className="text-xs font-semibold text-purple-600 uppercase mb-2">Review:</p>
            {answers.map((a, i) => (
              <div key={i} className={`text-sm mb-3 p-2 rounded-lg ${a.correct ? 'bg-green-50' : 'bg-red-50'}`}>
                <p className="font-medium text-gray-800">{a.correct ? '\u2705' : '\u274c'} {a.sentence}</p>
                <p className="text-gray-500 text-xs mt-1">{a.explanation}</p>
              </div>
            ))}
          </div>
  ...
  return (
    <section id="memory-lane">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-5 mb-5">
        <p className="font-semibold text-gray-800 text-lg leading-relaxed">&ldquo;{challenge.sentence}&rdquo;</p>
        ...
        <button
          ...
          className={`px-4 py-5 rounded-xl border-2 font-bold text-lg transition-all ${
            feedback
              ...
              : 'border-purple-200 hover:border-purple-400 hover:bg-purple-50/50 cursor-pointer text-purple-700'
          }`}
        >
          Imperfecto
            ...
            feedback
              ? challenge.correctTense === 'preterite'
                ...
                : selected === 'preterite'
                  ? 'border-red-300 bg-red-50 text-red-700'
                  : 'border-gray-200 opacity-40'
              : 'border-violet-200 hover:border-violet-400 hover:bg-violet-50/50 cursor-pointer text-violet-700'
          }`}
        >
          Pretérito
