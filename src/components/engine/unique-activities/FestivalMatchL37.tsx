...
export default function FestivalMatchL37({ onComplete }: { onComplete?: () => void }) {
  ...
  if (phase === 'start') {
    return (
      <section id="festival-match">
        ...
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read the description and match it to the correct Latin American festival!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127878;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} festivals. First, learn about each one. Then identify them from Spanish descriptions!</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per festival.</p>
          <button onClick={startStudy} className="px-8 py-3 bg-gradient-to-r from-rose-500 to-purple-500 text-white rounded-xl font-bold text-lg hover:from-rose-600 hover:to-purple-600 transition-all shadow-lg hover:shadow-xl">
            Learn the Festivals
          </button>
        </div>
      </section>
  ...
  if (phase === 'study') {
    return (
      <section id="festival-match">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Festival Match</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Study these festivals carefully. You&apos;ll need to identify them from Spanish descriptions!</p>
        <div className="grid sm:grid-cols-2 gap-4 mb-6">
          {FESTIVAL_MATCH_CHALLENGES.map((ch) => (
            <div key={ch.correctFestival} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">&#127879;</span>
                <h3 className="font-bold text-lg text-rose-700 dark:text-rose-400">{ch.correctFestival}</h3>
              </div>
    ...
    return (
      <section id="festival-match">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Festival expert!' : pct >= 50 ? 'Good job! Review the ones you missed.' : 'Keep practicing — these festivals are worth knowing!'}</p>
  ...
  return (
    <section id="festival-match">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Festival {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
        <div className="bg-rose-50 dark:bg-rose-950 border border-rose-200 dark:border-rose-800 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            <span className="text-2xl">&#127879;</span>
            <div>
              <p className="font-semibold text-gray-800 dark:text-gray-200 text-lg leading-relaxed">{challenge.description}</p>
              {feedback && <p className="text-sm text-gray-500 dark:text-gray-400 mt-2 italic">{challenge.english}</p>}
            </div>
          </div>
        </div>
        <p className="text-sm font-semibold text-gray-500 dark:text-gray-400 mb-3">Which festival is this?</p>
