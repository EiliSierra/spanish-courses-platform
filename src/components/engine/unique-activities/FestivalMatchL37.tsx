  ...
  if (phase === 'start') {
    return (
      <section id="festival-match">
        ...
        <p className="text-gray-600 mb-4">Read the description and match it to the correct Latin American festival!</p>
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          <div className="text-5xl mb-4">&#127878;</div>
          <p className="text-gray-600 mb-2">{totalRounds} festivals. Read the description in Spanish and identify each one.</p>
    ...
    return (
      <section id="festival-match">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
          ...
          <p className="text-gray-600 mb-4">{pct >= 80 ? 'Festival expert!' : pct >= 50 ? 'Good job! Review the ones you missed.' : 'Keep practicing — these festivals are worth knowing!'}</p>
  ...
  return (
    <section id="festival-match">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500">Festival {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        ...
        <p className="text-sm font-semibold text-gray-500 mb-3">Which festival is this?</p>
              ...
              <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
                className={`px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                  show ? isCorrect ? 'border-green-400 bg-green-50 text-green-800' : isSelected && !isCorrect ? 'border-red-300 bg-red-50 text-red-700' : 'border-gray-200 opacity-40'
                    : 'border-gray-200 hover:border-rose-400 hover:bg-rose-50/50 cursor-pointer'
                }`}>{opt}</button>
            )
          })}
        </div>
