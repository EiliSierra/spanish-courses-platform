...
import { HARVEST_PLANNER_CHALLENGES } from '@/lib/lessons/L9.3'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function HarvestPlannerL93({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = HARVEST_PLANNER_CHALLENGES[round]
  const totalRounds = HARVEST_PLANNER_CHALLENGES.length
  ...
  if (phase === 'start') {
    return (
      <section id="harvest-planner">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Harvest Planner</h2>
        <p className="text-gray-400 mb-4">Read each agricultural scenario and choose the best course of action!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#127807;</div>
          <p className="text-gray-400 mb-2">{totalRounds} scenarios. Pick the smartest farming decision for each challenge.</p>
    ...
    return (
      <section id="harvest-planner">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Expert farmer! You made all the right decisions!' : pct >= 50 ? 'Good instincts! Some farming decisions need more thought.' : 'Keep learning — sustainable farming takes practice!'}</p>
    ...
    <section id="harvest-planner">
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
          {feedback.correct ? 'Smart farming decision!' : `Best action: ${challenge.correctAction}`}
        </div>
      )}
    </section>
  )
