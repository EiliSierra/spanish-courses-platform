...
import { BLUEPRINT_READER_CHALLENGES } from '@/lib/lessons/L9.1'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function BlueprintReaderL91({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = BLUEPRINT_READER_CHALLENGES[round]
  const totalRounds = BLUEPRINT_READER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { description: challenge.description, correct: wasCorrect, chosen, answer: challenge.correctTerm }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctTerm
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="blueprint-reader">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Blueprint Reader</h2>
        <p className="text-gray-400 mb-4">Read each architectural description and pick the correct technical term!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128208;</div>
          <p className="text-gray-400 mb-2">{totalRounds} blueprint challenges. Identify the correct architectural term for each description.</p>
    ...
    return (
      <section id="blueprint-reader">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Master architect! You read blueprints like a pro!' : pct >= 50 ? 'Good work! Some terms need more review.' : 'Keep studying those architectural terms!'}</p>
  ...
  return (
    <section id="blueprint-reader">
      ...
      <div className="bg-amber-950 border border-amber-700 rounded-xl p-5 mb-5">
      ...
      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctTerm
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
          {feedback.correct ? 'Correct architectural term!' : `Correct: ${challenge.correctTerm}`}
        </div>
      )}
    </section>
  )
