...
import { PHILOSOPHY_CIRCLE_CHALLENGES } from '@/lib/lessons/L6.6'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 25000

export default function PhilosophyCircleL66({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ statement: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = PHILOSOPHY_CIRCLE_CHALLENGES[round]
  const totalRounds = PHILOSOPHY_CIRCLE_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { statement: challenge.statement, correct: wasCorrect, chosen, answer: challenge.correctAnswer }])
  ...
  if (phase === 'start') {
    return (
      <section id="philosophy-circle">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Philosophy Circle</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read philosophical statements and answer questions about their meaning. Think deeply!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#129300;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} philosophical challenges. Interpret statements from great thinkers.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">25 seconds per challenge.</p>
    ...
    return (
      <section id="philosophy-circle">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'A true philosopher! Brilliant reasoning.' : pct >= 50 ? 'Good philosophical thinking! Review the thinkers you missed.' : 'Philosophy takes practice. Revisit the concepts and try again!'}</p>
  ...
  return (
    <section id="philosophy-circle">
      ...
      <div className="bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 rounded-xl p-5 mb-3">
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg italic">&ldquo;{challenge.statement}&rdquo;</p>
      ...
      <div className="bg-violet-50 dark:bg-violet-950 border border-violet-200 dark:border-violet-800 rounded-xl p-4 mb-5">
        <p className="text-sm font-medium text-violet-800 dark:text-violet-800 dark:text-violet-200">{challenge.question}</p>
      </div>
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Excellent philosophical reasoning!' : `Correct: ${challenge.correctAnswer}`}
        </div>
      )}
    </section>
  )
