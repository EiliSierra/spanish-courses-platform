...
import { EMOTION_MAPPER_CHALLENGES } from '@/lib/lessons/L7.5'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function EmotionMapperL75({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = EMOTION_MAPPER_CHALLENGES[round]
  const totalRounds = EMOTION_MAPPER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { scenario: challenge.scenario, correct: wasCorrect, chosen, answer: challenge.correctEmotion }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctEmotion
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="emotion-mapper">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Emotion Mapper</h2>
        <p className="text-gray-400 mb-4">Read each emotional scenario and identify the correct psychological response or emotion. Think like a therapist!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128156;</div>
          <p className="text-gray-400 mb-2">{totalRounds} emotional scenarios. Choose the most accurate emotional response for each one.</p>
          <p className="text-sm text-gray-400 mb-6">22 seconds per scenario.</p>
    ...
    return (
      <section id="emotion-mapper">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Exceptional emotional intelligence! You read emotions like a therapist.' : pct >= 50 ? 'Good instincts! Study the emotions you missed.' : 'Keep exploring — emotional intelligence grows with practice!'}</p>
          <div className="bg-violet-950 rounded-xl p-4 mb-4 text-left space-y-2">
            ...
            {answers.map((a, i) => (
              <div key={i} className={`text-sm p-2 rounded-lg ${a.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
                ...
                {!a.correct && <p className="text-xs mt-1">Correct emotion: <span className="font-semibold">{a.answer}</span></p>}
              </div>
            ))}
          </div>
  ...
  return (
    <section id="emotion-mapper">
      ...
      <div className="bg-violet-950 border border-violet-800 rounded-xl p-5 mb-3">
      ...
      <div className="bg-purple-950 border border-purple-800 rounded-xl p-4 mb-5">
        <p className="text-sm font-medium text-purple-200">What emotion or response best describes this situation?</p>
      </div>

      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctEmotion
            ...
            <button key={opt} onClick={() => handleSelect(opt)} disabled={!!feedback}
              className={`w-full text-left px-4 py-4 rounded-xl border-2 font-medium text-sm transition-all ${
                show ? isCorrect ? 'border-green-400 bg-green-950 text-green-200' : isSelected && !isCorrect ? 'border-red-700 bg-red-950 text-red-700' : 'border-gray-700 opacity-40'
                  : 'border-gray-700 hover:border-violet-400 hover:bg-violet-950/50 cursor-pointer'
              }`}>{opt}</button>
          )
        })}
      </div>
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
          {feedback.correct ? 'Excellent emotional awareness!' : `Correct emotion: ${challenge.correctEmotion}`}
        </div>
      )}
    </section>
  )
