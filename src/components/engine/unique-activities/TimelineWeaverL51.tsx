  ...
  if (phase === 'start') {
    return (
      <section id="timeline-weaver">
        ...
        <p className="text-gray-400 mb-4">Put the events in chronological order. Pay attention to what had happened (pluperfect) vs. what happened (preterite)!</p>
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128205;</div>
          <p className="text-gray-400 mb-2">{totalRounds} narratives. Order the events from first to last.</p>
          <p className="text-sm text-gray-400 mb-6">Events using &quot;había&quot; (pluperfect) happened BEFORE the preterite events.</p>
    ...
    return (
      <section id="timeline-weaver">
        ...
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-400 mb-4">{pct >= 80 ? 'Master storyteller! Your sequencing is flawless.' : pct >= 50 ? 'Good work! Keep practicing event ordering.' : 'The pluperfect takes practice — try again!'}</p>
          <div className="bg-indigo-950 rounded-xl p-4 mb-4 text-left space-y-3">
  ...
  return (
    <section id="timeline-weaver">
      ...
      <div className="flex justify-between text-sm mb-3">
        <span className="text-gray-400">Narrative {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-indigo-950 border border-indigo-700 rounded-xl p-5 mb-5">
        <p className="font-semibold text-gray-200 text-lg">{challenge.narrative}</p>
        <p className="text-sm text-gray-400 mt-1 italic">{challenge.english}</p>
      </div>
      ...
      <div className="mb-4">
        ...
        <div className="space-y-2 min-h-[60px]">
          {userOrder.length === 0 && <p className="text-sm text-gray-400 italic">Click events below to add them in order...</p>}
          {userOrder.map((event, i) => {
            ...
            return (
              <div key={i} className={`flex items-center gap-2 p-3 rounded-lg border text-sm ${
                feedback ? (event === challenge.correctOrder[i] ? 'bg-green-950 border-green-700 text-green-800' : 'bg-red-950 border-red-700 text-red-800')
                  : isPluperfect ? 'bg-violet-950 border-violet-700' : 'bg-gray-800 border-gray-700'
              }`}>
      ...
      {remaining.length > 0 && !feedback && (
        <div className="mb-4">
          <p className="text-xs font-semibold text-gray-400 uppercase mb-2">Available Events (click to add):</p>
          <div className="space-y-2">
            {remaining.map((event) => (
              <button key={event} onClick={() => handleSelect(event)}
                className="w-full text-left px-4 py-3 rounded-xl border-2 border-gray-700 hover:border-indigo-400 hover:bg-indigo-950/50 text-sm font-medium transition-all cursor-pointer">
                {event}
              </button>
            ))}
          </div>
