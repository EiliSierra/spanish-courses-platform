...
import { RITUAL_GUIDE_CHALLENGES } from '@/lib/lessons/L9.7'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 20000

export default function RitualGuideL97({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = RITUAL_GUIDE_CHALLENGES[round]
  const totalRounds = RITUAL_GUIDE_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { scenario: challenge.description, correct: wasCorrect, chosen, answer: challenge.correctTradition }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctTradition
    setFeedback({ correct })
  ...
  if (phase === 'start') {
    return (
      <section id="ritual-guide">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Ritual Guide</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read the description of each religious or spiritual tradition and identify it correctly!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#9962;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} traditions described. Identify each one from the clues given.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">20 seconds per tradition.</p>
    ...
    return (
      <section id="ritual-guide">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Deep cultural knowledge! You know these traditions beautifully.' : pct >= 50 ? 'Good cultural awareness! Review the traditions you missed.' : 'Keep exploring the rich world of Hispanic spiritual traditions!'}</p>
          <div className="bg-amber-50 dark:bg-amber-950 rounded-xl p-4 mb-4 text-left space-y-2">
            <p className="text-xs font-semibold text-amber-600 uppercase mb-2">Your Answers:</p>
            {answers.map((a, i) => (
  ...
  return (
    <section id="ritual-guide">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Tradition {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-orange-50 dark:bg-orange-950 border border-orange-200 dark:border-orange-800 rounded-xl p-4 mb-5">
        <p className="text-sm font-medium text-orange-800 dark:text-orange-800 dark:text-orange-200">Which tradition or practice is being described?</p>
      </div>

      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctTradition
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Correct tradition identified!' : `Correct: ${challenge.correctTradition}`}
        </div>
      )}
    </section>
  )
