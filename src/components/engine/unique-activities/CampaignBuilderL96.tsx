...
import { CAMPAIGN_BUILDER_CHALLENGES } from '@/lib/lessons/L9.6'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function CampaignBuilderL96({ onComplete }: { onComplete?: () => void }) {
  ...
  const challenge = CAMPAIGN_BUILDER_CHALLENGES[round]
  const totalRounds = CAMPAIGN_BUILDER_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { scenario: challenge.brief, correct: wasCorrect, chosen, answer: challenge.correctStrategy }])
  ...
  if (phase === 'start') {
    return (
      <section id="campaign-builder">
        <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Campaign Builder</h2>
        <p className="text-gray-500 dark:text-gray-400 mb-4">Read each marketing brief and choose the best strategy. Think like a digital marketing expert!</p>
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          <div className="text-5xl mb-4">&#128200;</div>
          <p className="text-gray-500 dark:text-gray-400 mb-2">{totalRounds} marketing briefs. Choose the most effective strategy for each scenario.</p>
          <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">22 seconds per brief.</p>
          <button onClick={startGame} className="px-8 py-3 bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-xl font-bold text-lg hover:from-purple-600 hover:to-blue-600 transition-all shadow-lg hover:shadow-xl">
            Start Building
          </button>
        </div>
      </section>
    ...
    return (
      <section id="campaign-builder">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-8 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mb-4">{pct >= 80 ? 'Brilliant marketing strategist! Your campaigns would thrive.' : pct >= 50 ? 'Good marketing sense! Review the strategies you missed.' : 'Keep learning digital marketing — data-driven decisions win!'}</p>
          <div className="bg-purple-50 dark:bg-purple-950 rounded-xl p-4 mb-4 text-left space-y-2">
            <p className="text-xs font-semibold text-purple-600 uppercase mb-2">Your Strategies:</p>
            {answers.map((a, i) => (
  ...
  return (
    <section id="campaign-builder">
      ...
      <div className="flex justify-between text-sm mb-2">
        <span className="text-gray-500 dark:text-gray-400">Brief {round + 1} of {totalRounds}</span>
      ...
      <div className="bg-purple-50 dark:bg-purple-950 border border-purple-200 dark:border-purple-800 rounded-xl p-5 mb-3">
        <p className="font-semibold text-gray-700 dark:text-gray-200 text-lg">{challenge.brief}</p>
      ...
      <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-xl p-4 mb-5">
        <p className="text-sm font-medium text-blue-800 dark:text-blue-800 dark:text-blue-200">Which marketing strategy is most effective?</p>
      </div>
      ...
      {feedback && (
        <div className={`text-center text-sm font-semibold p-3 rounded-lg mt-4 ${feedback.correct ? 'bg-green-50 dark:bg-green-950 text-green-800 dark:text-green-200' : 'bg-red-50 dark:bg-red-950 text-red-800 dark:text-red-200'}`}>
          {feedback.correct ? 'Excellent marketing strategy!' : `Correct strategy: ${challenge.correctStrategy}`}
        </div>
      )}
    </section>
  )
