...
import { TRANSLATOR_BOOTH_CHALLENGES } from '@/lib/lessons/L6.5'
import { spawnConfetti } from '@/components/ui/Confetti'

const TIME_LIMIT = 22000

export default function TranslatorBoothL65({ onComplete }: { onComplete?: () => void }) {
  ...
  const [answers, setAnswers] = useState<{ sourceText: string; correct: boolean; chosen: string; answer: string }[]>([])
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const scoreRef = useRef(0)

  const challenge = TRANSLATOR_BOOTH_CHALLENGES[round]
  const totalRounds = TRANSLATOR_BOOTH_CHALLENGES.length
  ...
  const advanceRound = useCallback((wasCorrect: boolean, chosen: string) => {
    ...
    setAnswers(prev => [...prev, { sourceText: challenge.sourceText, correct: wasCorrect, chosen, answer: challenge.correctTranslation }])
  ...
  const handleSelect = useCallback((opt: string) => {
    ...
    clearTimer()
    setSelected(opt)
    const correct = opt === challenge.correctTranslation
    setFeedback({ correct })
    ...
    <section id="translator-booth">
      ...
      <div className="space-y-3">
        {challenge.options.map((opt) => {
          const isCorrect = opt === challenge.correctTranslation
