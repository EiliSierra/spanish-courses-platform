          ...
          <div className="grid sm:grid-cols-2 gap-3 mb-4">
            {PROFILE_OPTIONS.map((opt) => (
              <button
                ...
                className="text-left p-4 rounded-xl border border-gray-200 dark:border-gray-700 hover:border-indigo-300 hover:bg-indigo-950/50 transition-all group"
              >
                <span className="font-semibold text-gray-700 dark:text-gray-200 group-hover:text-indigo-700 text-sm">{opt.label}</span>
              </button>
            ))}
          </div>
          ...
          {!showCustom ? (
            ...
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
      ...
      {loading && (
        <div className="flex items-center justify-center py-12">
          <div className="flex items-center gap-3">
            ...
            <span className="text-gray-500 dark:text-gray-400 text-sm">AI is personalizing vocabulary for you...</span>
          </div>
        </div>
      )}
      ...
      {sentences.length > 0 && (
        <div>
          <div className="space-y-3 mb-4">
            {sentences.map((s, i) => (
              <div key={i} className="bg-white dark:bg-gray-800 rounded-xl border border-indigo-800 p-4 hover:shadow-sm transition-shadow">
                <div className="flex items-center gap-2 mb-2">
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold bg-indigo-100 text-indigo-700">
                    {s.word}
                  </span>
                  <span className="text-xs text-gray-500 dark:text-gray-400">{s.connection}</span>
                </div>
                <p className="text-gray-900 dark:text-gray-100 font-medium">{s.spanish}</p>
