...
export default function PronunciationTips({ tips }: { tips: PronunciationTip[] }) {
  ...
  return (
    <section id="pronunciation-tips">
      ...
      <p className="text-gray-400 mb-5">Key patterns for this lesson&apos;s vocabulary.</p>
      <div className="space-y-3">
        {tips.map((tip, i) => (
          <div key={tip.title} className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden">
            <button onClick={() => setOpenIdx(openIdx === i ? null : i)}
              className="w-full flex items-center gap-3 px-5 py-4 text-left hover:bg-gray-900 transition-colors">
              ...
              <span className="font-semibold text-gray-200 flex-1">{tip.title}</span>
            ...
            {openIdx === i && (
              <div className="px-5 pb-4 pl-16">
                ...
                <p className="text-gray-400 text-sm mb-3">{tip.content}</p>
                <div className="bg-amber-950 border-l-3 border-amber-400 px-4 py-2 rounded-r-lg text-sm text-amber-900 font-medium">{tip.example}</div>
              </div>
            )}
          </div>
        ))}
