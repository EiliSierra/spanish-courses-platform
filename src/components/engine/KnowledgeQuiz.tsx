...
export default function KnowledgeQuiz({ questions, onComplete }: KnowledgeQuizProps) {
  ...
  return (
    <section id="knowledge-check">
      ...
      <p className="text-gray-400 mb-2">Answer all {questions.length} questions. You need 70% to pass.</p>
      ...
      {showFinal && (
        <div className="mt-8 bg-gray-800 rounded-xl border border-gray-700 p-8 text-center">
          {(() => {
            ...
            return (<>
              ...
              <p className="text-gray-400 mb-4">{correctCount} of {questions.length} correct</p>
...
function QuizCard({ question: q, isAnswered, isCorrect, onAnswer }: { question: QuizQuestion; isAnswered: boolean; isCorrect?: boolean; onAnswer: (correct: boolean) => void }) {
  ...
  const typeBg = q.type === 'mc' ? 'bg-blue-900 text-blue-200' : q.type === 'tf' ? 'bg-purple-900 text-purple-200' : 'bg-amber-900 text-amber-200'
  ...
  return (
    <div className={`bg-gray-800 rounded-xl border p-6 transition-all ${isAnswered ? isCorrect ? 'border-green-500 bg-green-950/30' : 'border-red-500 bg-red-950/30' : 'border-gray-700'}`}>
      ...
      <p className="font-medium text-gray-200 mb-4">{q.text}</p>
            ...
            return (
              <button key={i} onClick={() => { if (!isAnswered) { setSelectedOption(i); onAnswer(i === (q.answer as number)) } }} disabled={isAnswered}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg border-2 text-left transition-all ${
                  isAnswered ? isThisCorrect ? 'border-green-500 bg-green-950 text-green-200' : isSelected ? 'border-red-500 bg-red-950 text-red-200' : 'border-gray-700 opacity-50' : 'border-gray-700 hover:border-blue-500 hover:bg-blue-950 cursor-pointer'
                }`}>
                <span className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${isAnswered && isThisCorrect ? 'bg-green-500 text-white' : isAnswered && isSelected ? 'bg-red-500 text-white' : 'bg-gray-800 text-gray-400'}`}>
                  {['A', 'B', 'C', 'D'][i]}
                </span>
          ...
          {[true, false].map((val) => (
            <button key={String(val)} onClick={() => { if (!isAnswered) onAnswer(val === (q.answer as boolean)) }} disabled={isAnswered}
              className={`flex-1 px-4 py-3 rounded-lg border-2 font-medium text-center transition-all ${
                isAnswered ? val === (q.answer as boolean) ? 'border-green-500 bg-green-950 text-green-200' : 'border-gray-700 opacity-50' : 'border-gray-700 hover:border-blue-500 hover:bg-blue-950 cursor-pointer text-gray-200'
              }`}>
              {val ? 'True' : 'False'}
            </button>
          ))}
        ...
        <div className="flex gap-3">
          <input type="text" value={fillInput} onChange={(e) => setFillInput(e.target.value)}
            ...
            className={`w-32 h-12 text-center text-xl font-bold rounded-lg border-2 outline-none transition-colors ${
              isAnswered ? isCorrect ? 'border-green-500 bg-green-950 text-green-200' : 'border-red-500 bg-red-950 text-red-200' : 'border-gray-600 bg-gray-900 text-gray-100 focus:border-blue-400'
            }`} />
      ...
      {isAnswered && (
        <div className={`mt-3 text-sm p-3 rounded-lg ${isCorrect ? 'bg-green-950 text-green-200' : 'bg-red-950 text-red-200'}`}>
