      ...
      <div className="lg:ml-[280px]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 space-y-6">
          ...
          {data.priorKnowledge && data.priorKnowledge.length > 0 && (
            <div className={`section-card ${sc['prior-knowledge']?.bg ?? 'bg-indigo-50/30'} ${sc['prior-knowledge']?.border ?? 'border-indigo-100'}`}>
            ...
            <section id="objectives">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {data.objectives.map((obj) => (
                  <div key={obj.title} className="flex gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
          ...
          {data.personalizedVocab && data.personalizedVocab.length > 0 && (
            <div className={`section-card ${sc['personalized-context']?.bg ?? 'bg-violet-50/30'} ${sc['personalized-context']?.border ?? 'border-violet-100'}`}>
          ...
          {data.pronunciationChallenges && data.pronunciationChallenges.length > 0 && (
            <div className={`section-card ${sc['pronunciation-practice']?.bg ?? 'bg-blue-50/30'} ${sc['pronunciation-practice']?.border ?? 'border-blue-200'}`}>
