...
import { PHRASES, type PhraseData } from '@/lib/lesson-data-L12'

const FC_GROUPS = [
  {
    key: 'essentials', label: 'Essentials',
    audioKeys: ['hola', 'adios', 'gracias', 'por-favor', 'me-llamo', 'mucho-gusto', 'buenos-dias', 'hasta-luego', 'como-esta', 'disculpe', 'no-entiendo', 'donde-esta'],
  },
  {
    key: 'building', label: 'Building',
    audioKeys: ['buenas-tardes', 'buenas-noches', 'que-tal', 'hasta-manana', 'nos-vemos', 'chao', 'muchas-gracias', 'lo-siento', 'perdon', 'encantado', 'soy-de', 'como-te-llamas'],
  },
  {
    key: 'challenge', label: 'Challenge',
    audioKeys: ['hasta-pronto', 'mil-gracias', 'puede-ayudarme', 'habla-ingles', 'puede-repetir', 'no-se', 'no-hablo-espanol', 'mas-despacio', 'como-se-llama', 'de-donde-eres', 'de-donde-es', 'muy-bien', 'mas-o-menos', 'soy', 'bien-gracias', 'el-gusto-es-mio'],
  },
]
...
const catColor: Record<string, string> = {
  greeting: 'from-emerald-600 to-teal-600 border-emerald-400',
  farewell: 'from-blue-600 to-cyan-600 border-blue-400',
  polite: 'from-purple-600 to-indigo-600 border-purple-400',
  introduction: 'from-amber-600 to-orange-600 border-amber-400',
  question: 'from-rose-600 to-pink-600 border-rose-400',
}

export default function FlashcardGridL12() {
  ...
  const playAudio = useCallback((audio: string) => {
    new Audio(`/audio/L1.2/phrases/${audio}.mp3`).play().catch(() => {})
  }, [])
    ...
    <section id="flashcards">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-5">
        {cards.map((item) => {
          ...
          const color = catColor[item.category] ?? catColor.greeting
