...
import { MATCH_PAIRS_L12 } from '@/lib/lesson-data-L12'
...
export default function MatchingGameL12({ onComplete }: { onComplete?: () => void }) {
  const total = MATCH_PAIRS_L12.length
  const [score, setScore] = useState(0)
  const [matched, setMatched] = useState<Set<string>>(new Set())
  const [wrongZone, setWrongZone] = useState<string | null>(null)
  const [dragging, setDragging] = useState<string | null>(null)
  const [shuffledLeft] = useState(() => shuffle(MATCH_PAIRS_L12))
  const [shuffledRight] = useState(() => shuffle(MATCH_PAIRS_L12))
  const scoreRef = useRef(0)

  const handleDrop = useCallback(
    (leftText: string, targetRight: string) => {
      const pair = MATCH_PAIRS_L12.find((p) => p.left === leftText)
  ...
  return (
    <section id="matching-game">
      ...
      <p className="text-gray-600 mb-2">Drag each Spanish phrase to its English translation. <strong>Score: {score}/{total}</strong></p>

      <div className="grid md:grid-cols-2 gap-6 mb-4">
        <div className="space-y-2">
          <div className="text-sm font-semibold text-gray-500 mb-2">Spanish</div>
            ...
            <div
              ...
              className={`px-4 py-3 rounded-lg font-medium text-center transition-all select-none ${
                matched.has(pair.left)
                  ...
                  : 'bg-white border-2 border-emerald-200 text-emerald-800 cursor-grab hover:border-emerald-400 hover:shadow-sm active:cursor-grabbing'
              }`}
            >
              {pair.left}
            </div>
        ...
        <div className="space-y-2">
          ...
          {shuffledRight.map((pair) => {
            const isMatched = matched.has(MATCH_PAIRS_L12.find((p) => p.right === pair.right)?.left ?? '')
              ...
              >
                ...
                {isMatched && (
                  <span className="text-sm font-normal ml-2 text-green-600">
                    ✓ {MATCH_PAIRS_L12.find((p) => p.right === pair.right)?.left}
                  </span>
                )}
              </div>
            )
