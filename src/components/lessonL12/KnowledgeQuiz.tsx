...
import { QUIZ_L12 } from '@/lib/lesson-data-L12'
import { spawnConfetti } from '@/components/ui/Confetti'

type QuizQ = typeof QUIZ_L12[number]

export default function KnowledgeQuizL12({ onComplete }: { onComplete?: (score: number, max: number) => void }) {
  ...
  const progressPct = Math.round((answeredCount / QUIZ_L12.length) * 100)
  ...
  const handleAnswer = useCallback((qId: number, isCorrect: boolean) => {
    ...
    if (answeredCount + 1 === QUIZ_L12.length) {
      setTimeout(() => {
        setShowFinal(true)
        onComplete?.(newCorrect, QUIZ_L12.length)
        if (Math.round((newCorrect / QUIZ_L12.length) * 100) >= 80) spawnConfetti(window.innerWidth / 2, 200, 60)
      }, 800)
    }
  ...
  return (
    <section id="knowledge-check">
      ...
      <p className="text-gray-600 mb-2">Answer all {QUIZ_L12.length} questions. You need 70% to pass.</p>

      <div className="mb-6">
        <div className="flex justify-between text-sm mb-1">
          <span className="text-gray-500">{answeredCount < QUIZ_L12.length ? `Question ${answeredCount + 1} of ${QUIZ_L12.length}` : `${QUIZ_L12.length} of ${QUIZ_L12.length} answered`}</span>
      ...
      {showFinal && (
        <div className="mt-8 bg-white rounded-xl border border-gray-200 p-8 text-center">
          {(() => {
            const pct = Math.round((correctCount / QUIZ_L12.length) * 100)
            const stars = pct >= 90 ? 3 : pct >= 60 ? 2 : pct >= 30 ? 1 : 0
            return (<>
              <div className="text-4xl mb-2">{'⭐'.repeat(stars)}{'☆'.repeat(3 - stars)}</div>
              <div className="text-4xl font-bold text-blue-600 mb-1">{pct}%</div>
              <p className="text-gray-600 mb-4">{correctCount} of {QUIZ_L12.length} correct</p>
...
function QuizCard({ question: q, isAnswered, isCorrect, onAnswer }: { question: QuizQ; isAnswered: boolean; isCorrect?: boolean; onAnswer: (correct: boolean) => void }) {
  ...
  return (
    <div className={`bg-white rounded-xl border p-6 transition-all ${isAnswered ? isCorrect ? 'border-green-300 bg-green-50/30' : 'border-red-300 bg-red-50/30' : 'border-gray-200'}`}>
      ...
      {q.type === 'mc' && 'options' in q && (q as { options: string[] }).options && (
      ...
      {q.type === 'fill' && (
        <div className="flex gap-3">
          <input type="text" value={fillInput} onChange={(e) => setFillInput(e.target.value)}
            onKeyDown={(e) => { if (e.key === 'Enter' && !isAnswered && fillInput.trim()) { const n = (s: string) => s.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); onAnswer(n(fillInput) === n(q.answer as string)) } }}
          ...
          <button onClick={() => { if (!isAnswered && fillInput.trim()) { const n = (s: string) => s.trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''); onAnswer(n(fillInput) === n(q.answer as string)) } }}
