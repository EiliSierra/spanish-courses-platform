...
import { SORT_ACTIVITIES_L12 } from '@/lib/lesson-data-L12'
...
export default function SortingActivityL12({ onComplete }: { onComplete?: () => void }) {
  const [activeTab, setActiveTab] = useState(0)
  const [placements, setPlacements] = useState<Record<number, string>>({})
  const [checked, setChecked] = useState(false)
  const [results, setResults] = useState<Record<number, boolean>>({})
  const [items, setItems] = useState(() => shuffle(SORT_ACTIVITIES_L12[0].items.map((item, i) => ({ ...item, idx: i }))))
  const [selected, setSelected] = useState<number | null>(null)
  const [dragging, setDragging] = useState<number | null>(null)

  const activity = SORT_ACTIVITIES_L12[activeTab]
  ...
  const switchTab = useCallback((tabIdx: number) => {
    ...
    setResults({})
    setSelected(null)
    setItems(shuffle(SORT_ACTIVITIES_L12[tabIdx].items.map((item, i) => ({ ...item, idx: i }))))
  }, [])
  ...
  return (
    <section id="phrase-sorting">
      <h2 className="text-2xl font-bold font-[family-name:var(--font-inter)] mb-2">Sort the Phrases</h2>
      <p className="text-gray-600 mb-4">Drag items into the correct category, or click to select then click a bucket.</p>

      <div className="flex gap-2 mb-5 flex-wrap">
        {SORT_ACTIVITIES_L12.map((act, i) => (
